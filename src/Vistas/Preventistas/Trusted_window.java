...
public class Trusted_window extends javax.swing.JFrame {
    ...
    static int empId, indice = 0, estado = 0;
    ...
    //Tabla
    ModeloEditablePorFila trusted_tablemod = new ModeloEditablePorFila(new String[]{"Nº", "Saldo", "Cliente", "Fecha"}, 0) {
        private final int editableRow = -1;
    };
    //Tabla

    public Trusted_window(int empId, String empName) throws SQLException {
        initComponents();
        Trusted_window.empId = empId;
        Trusted_window.empName = empName;
        this.setLocationRelativeTo(null);
        Clases.General_configurations.Icon(this);
        Clases.General_configurations.MaxMinWindow(this);
        Clases.General_configurations.table_configuration(trusted_table, trusted_tablemod);
        Clases.Trusted_window.ShowTrustedGeneral(con, trusted_tablemod, empId, trusted_table, lbl_general, lbl_pending, lbl_accredited);
        if (trusted_table.getRowCount() == 0) {
        ...
        trusted_table.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                // Detectar doble click
                if (e.getClickCount() == 2 && trusted_table.getSelectedRow() != -1) {
                    int fila = trusted_table.getSelectedRow();

                    // Tomamos datos de la fila seleccionada
                    Object id = trusted_table.getValueAt(fila, 0);
                    int idOf = Integer.parseInt(id.toString());

                    Object saldo = trusted_table.getValueAt(fila, 1);
                    int saldoOf = Integer.parseInt(saldo.toString());

                    Object fecha = trusted_table.getValueAt(fila, 3);
                    String fechaOf = String.valueOf(fecha);

                    try {
                        Clases.Trusted_window.Verification(con, saldoOf, idOf, fechaOf, Trusted_window.this);
                        if (indice == 0) {
                            if (estado == 1) {
                                try {
                                    trusted_tablemod.setRowCount(0);
                                    Clases.Trusted_window.ShowTrustedPending(con, trusted_tablemod, empId, trusted_table, lbl_general, lbl_pending, lbl_accredited);
                                    verificacionpdf = 0;
                                    if (trusted_table.getRowCount() == 0) {
                                        JOptionPane.showMessageDialog(null, "¡No se encontró ningún Fiado pendiente!");
                                        create_pdf_button.setEnabled(false);
                                    }
                                ...
                                try {
                                    ...
                                    trusted_table.setDefaultRenderer(Object.class, centerRenderer);
                                    if (trusted_table.getRowCount() == 0) {
                                        JOptionPane.showMessageDialog(null, "¡No se encontró ningún Fiado!");
                                        create_pdf_button.setEnabled(false);
                                        show_pending_button.setEnabled(false);
                            ...
                            verificacionpdf = 1;
                            try {
                                Clases.Trusted_window.ShowTrustedMonth(con, trusted_tablemod, empId, trusted_table, lbl_general, lbl_accredited, lbl_pending, indice);
                                if (trusted_table.getRowCount() == 0) {
                                    JOptionPane.showMessageDialog(null, "¡No se encontró ningún Fiado en este mes!");
                                    create_pdf_button.setEnabled(false);
                                    show_pending_button.setEnabled(false);
                                    verificacionpdf = 1;
                                }
    ...
    private void initComponents() {
        ...
        trusted_table = new javax.swing.JTable();
        ...
        lbl_title.setText("FIADOS");

        trusted_table.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, null, null, null},
                {null, null, null, null},
                {null, null, null, null},
        ...
        jScrollPane1.setViewportView(trusted_table);
        ...
        month_cb.addAncestorListener(new javax.swing.event.AncestorListener() {
            public void ancestorMoved(javax.swing.event.AncestorEvent evt) {
            }
            public void ancestorAdded(javax.swing.event.AncestorEvent evt) {
                month_cbAncestorAdded(evt);
            }
    ...
    private void show_pending_buttonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_show_pending_buttonActionPerformed
        estado = show_pending_button.isSelected() ? 1 : 0;
        if (estado == 1) {
    ...
    private void create_pdf_buttonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_create_pdf_buttonActionPerformed
        int opcion = JOptionPane.showConfirmDialog(null, "¿Deseas Guardar en PDF?", "Confirmar acción",
                JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE);
        ...
        String rutaBase = "C:\\TOC";
        String rutaFiados = rutaBase + "\\Fiados";
        String rutaMensual = rutaFiados + "\\" + nombreMes + " " + añoString;

        // 3. Crear carpetas (usando un pequeño método auxiliar o validación simple)
        File carpetaFinal = new File(rutaMensual);
        if (!carpetaFinal.exists()) {
            if (carpetaFinal.mkdirs()) {
                System.out.println("✅ Carpetas creadas hasta: " + rutaMensual);
            } else {
                System.out.println("❌ Error al crear ruta: " + rutaMensual);
                return;
            }
        }

        // 4. Generar PDF
        String direccion = rutaMensual + "\\FiadosPend_Preventista_" + empName + "_" + TIMESTAMP + "_" + nombreMes + ".pdf";

        try {
            Clases.PDF.save_PDF_Trusted(con, direccion, empId, indice, verificacionpdf);
        } catch (Exception ex) {
            ex.printStackTrace(); // Siempre es mejor ver el error que dejarlo vacío
        }
    }//GEN-LAST:event_create_pdf_buttonActionPerformed

    private void month_cbAncestorAdded(javax.swing.event.AncestorEvent evt) {//GEN-FIRST:event_month_cbAncestorAdded
        // TODO add your handling code here:
    }//GEN-LAST:event_month_cbAncestorAdded
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            ...
            public void run() {
                try {
                    new Trusted_window(0, null).setVisible(true);
                } catch (SQLException ex) {
                    Logger.getLogger(Trusted_window.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
    ...
    private javax.swing.JTable trusted_table;
